...
public class Physics {
    ...
    static func boxIntersectsSphere(box: BoundingBox, sphere: BoundingSphere) -> Bool {
        ...
        if sphere.center.x < box.min.x { dist_squared -= pow(sphere.center.x - box.min.x, 2) } else if sphere.center.x > box.max.x { dist_squared -= pow(sphere.center.x - box.max.x, 2) }
        if sphere.center.y < box.min.y { dist_squared -= pow(sphere.center.y - box.min.y, 2) } else if sphere.center.y > box.max.y { dist_squared -= pow(sphere.center.y - box.max.y, 2) }
        if sphere.center.z < box.min.z { dist_squared -= pow(sphere.center.z - box.min.z, 2) } else if sphere.center.z > box.max.z { dist_squared -= pow(sphere.center.z - box.max.z, 2) }
